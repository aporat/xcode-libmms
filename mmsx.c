...
mmsx_t *mmsx_connect(mms_io_t *io, void *data, const char *url, const char *host, const char * uri,const char *query, int port,char *scheme,int bandwidth)
{
  ...
  if (!mmsx)
	...
	// AJS
  if ( port==80 || !strncmp(scheme,"http",4))
  {
    mmsx->connection_h = mmsh_connect(io, data, url,host, uri,query,port, bandwidth);
    if (mmsx->connection_h)
      return mmsx;
    
    mmsx->connection = mms_connect(io, data, url, host, uri,query,port, bandwidth);
    if (mmsx->connection)
      return mmsx;
	...
	//
	// AJS - if there still isn't a connection, try port 1755
	//
	if (!mmsx->connection && port!=1755 && port!=0)
	{
		mmsx->connection = mms_connect(io, data, url, host, uri,query,1755, bandwidth);
		if (mmsx->connection)
			return mmsx;
	}
...
int mmsx_get_eos (mmsx_t *mmsx)
{
	if(mmsx->connection)
		return mms_get_eos(mmsx->connection);
	else
		return mmsh_get_eos(mmsx->connection_h);
}
