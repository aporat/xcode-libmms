...
mmsh_t*   mmsh_connect (mms_io_t *io, void *data, const char *url, const char *host, const char *uri,const  char *query,int pport, int bandwidth)
{
  ...
  report_progress (NULL, 0);
	  ...
	  this->proto = strdup("mms");
	  this->connect_host= strdup(host);//"2.uni1.salem.streamaudio.com";
	  this->http_host= strdup(host);//"2.uni1.salem.streamaudio.com";
	  this->http_port= pport; // 80;
	  this->connect_port=pport;// 80;
	  this->host_user= NULL;
	  this->host_password = NULL;
	  this->uri= strdup(newuri);//"/KDOW_AM";
